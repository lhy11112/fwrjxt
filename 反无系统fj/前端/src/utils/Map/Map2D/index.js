...
export default {
    ...
    init(containerId) {
     ...
     initMapLayers() {
        // 重置图层变量，避免指向旧实例
        this.kxMarkerLayer = null;
        this.kxRectLayer = null;
        this.xdMarkerLayer = null;
        // 清空全局变量，强制重新创建
        window.kxMarkerLayer = null;
        window.kxRectLayer = null;
        window.rect = null;
    },
                ...
                if(that.kxFlag && isDrawing && startPoint){
                    ...
                    zbmbApi.queryBySquareArea({
                        ...
                        if (res.success && res.result && res.result.length) {
                            // 清除之前的标记
                            if (that.clusterMarkers) {
                                that.clusterMarkers.clearLayers();
                            }
                            // 初始化标记图层
                            if (!that.kxMarkerLayer) {
                                that.kxMarkerLayer = window.L.layerGroup([]);
                                that.kxMarkerLayer.addTo(that.map);
                            } else {
                                that.kxMarkerLayer.clearLayers();
                            }
                    ...
                    const closeIcon = L.divIcon({
                        ...
                        className: '',
                        iconSize: [32, 32],
                        iconAnchor: [32, 0] // 锚点调整：从右上角向内偏移，避免遮盖标注
                    });
                    
                    const rectBounds = window.rect.getBounds();
                    const northEast = rectBounds.getNorthEast();
                    const closeMarker = L.marker([northEast.lat, northEast.lng], {
                        icon: closeIcon,
                        interactive: true,
                        zIndexOffset: 1000
                    }).addTo(window.kxRectLayer);
                    
                    // 阻止矩形的点击事件冒泡
                    window.rect.on('click', function(e) {
                        L.DomEvent.stop(e);
                    });
