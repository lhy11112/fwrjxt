...
<div class="largeContent">
    ...
    <div id="bigScreen">
      ...
      <div class="scBox" v-show="scFlag">
        ...
        <div class="icons">
          <el-icon @click="savePoint" title="保存当前视角"><View /></el-icon>
          <el-icon @click="saveSubmit" title="保存"><FolderChecked /></el-icon>
        ...
        <div class="scContent">
          <el-table
                ...
                class="custom-table"
                @selectionChange="selectionScChange"
                v-loading="loading"
              >
                <el-table-column type="selection" width="50"></el-table-column>
                <el-table-column
                  v-for="(item, index) in scjColumnData"
                  :prop="item.prop"
                  :label="item.label"
                  :key="index"
                  :width="item.width"
                >
                  <template #default="scope">
                    <span @click="goSt(scope.row)" style="cursor:pointer;">{{scope.row.mc}}</span>
                  </template>
                </el-table-column>
                <el-table-column fixed="right" label="操作" width="120">
                  <template #default="scope">
                    <el-button
                      ...
                      type="primary"
                      size="small"
                      @click.prevent="remove(scope.row,scope.$index)"
                    >
                      删除
                    </el-button>
                    <el-button
  ...
  <shituDialog v-if="stVisible" ref="stRef" @successClick="shituSuccess"></shituDialog>
...
const saveSubmit = () => {
  ...
  if (mapChange.value) {
    console.log('xxxxxxx');
    saveParams.value.csz.scData2 = scData.value && scData.value.length?scData.value:[]
  }else{
    console.log('aaaaaaaaa');
    saveParams.value.csz.scData1 = scData.value && scData.value.length?scData.value:[]
  }
...
const getScjData = () => {
  ...
  try {
    window.API.scj.list(params).then(res=>{
      if(res.code==200 && res.success){
        if(res.result.records && res.result.records.length && res.result.records[0].csz){
          let csz = JSON.parse(res.result.records[0].csz);
          Object.assign(saveParams.value,res.result.records[0]);
          saveParams.value.csz = {
            scData1: csz.scData1 ? csz.scData1 : [],
            scData2: csz.scData2 ? csz.scData2 : []
          };
          if (mapChange.value) {
            scData.value = csz.scData2 ? csz.scData2 : [];
          }else{
            scData.value = csz.scData1 ? csz.scData1 : [];
          }
        }else{
          scData.value = [];
          saveParams.value = {
  ...
  // let csz = JSON.parse("{\"scData1\":[{\"mc\":\"视角X:26.07312838384548,视角Y:119.30603027343751,视角Z:9\",\"jd\":119.30603027343751,\"wd\":26.07312838384548,\"zoom\":9,\"id\":5619291.619838855}],\"scData2\":[]}");
  //         saveParams.value.csz.scData1 = csz.scData1?csz.scData1:[];
  //         saveParams.value.csz.scData2 = csz.scData2?csz.scData2:[];
...
const goSt = (data) => {
  if (mapChange.value) {
    ...
    window.Map2D.map.flyTo([Number(data.wd),Number(data.jd)],data.zoom);
  }
...
const shituSuccess = (data,mode) => {
  ...
  if(mode=='add'){
    scData.value.push(data)
  }else if(mode=='edit'){
    scData.value.forEach((item,index)=>{
      if(item.id == data.id){
        console.log(index,1,data);
        scData.value.splice(index,1,data)
      }
    })
