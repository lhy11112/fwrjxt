<template>
  <div class="toggleBottom" id="bottomMenu">
    <!-- <div class="tabsBottom">
      ...
      <span @click="tabsIndex=2" :class="tabsIndex==2?'tabsActive':''">时间轴</span>
    </div> -->
  ...
  <mbfxDialog v-if="mbfxVisible" @success="mbfxSuccess" ref="mbfxRef" @closed="mbfxClose"></mbfxDialog>
...
import gjfxcsDialog from "./gjfxcs.vue"
import gjfxDialog from "./gjfx.vue"
...
onMounted(()=>{
  ...
  // 获取所有闪烁层
   ...
   eventBus.on("timeline",(data)=>{
    tabsIndex.value = data;
   })
})

if (window.location.href.includes("zbxx")) {
  currentMenuIndex.value = 0;
}else if(window.location.href.includes("fwzf")){
  currentMenuIndex.value = 1;
}else if(window.location.href.includes("bxpz")){
  currentMenuIndex.value = 2;
}else if(window.location.href.includes("zymb")){
  currentMenuIndex.value = 3;
}else if(window.location.href.includes("zhby")){
...
const menusClick = (index) => {
  ...
  if(index==0){
    ...
    timer2.value=null;
    clearLayer1()
    router.push('/zbxx')// 装备信息
  }else if(index==1){
    router.push('/fwzf') //反无战法
    clearInterval(timer2.value)
    timer2.value=null;
    clearLayer1()
  ...
  //   // nextTick(()=>{
  //   //   gjfxcsRef.value.open(true,xdData.value)
  //   // })
  //   router.push('/cesiumAir')
  //   clearInterval(timer2.value)
  //   timer2.value=null;
  //   clearLayer1()
  //   timer2.value =setInterval(()=>{
