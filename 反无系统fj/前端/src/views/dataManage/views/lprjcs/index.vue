  ...
  <div style="width:100%;height:100%;">
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('录屏软件参数')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button> -->
                <!-- <el-button
                  type="primary"
                  @click="table_export('录屏软件参数_导入模板','template')"
                >
                  <el-icon><Download /></el-icon>下载模板
                </el-button> -->
                <el-button
                  type="primary"
                  @click="table_download"
                >
                  <el-icon><Download /></el-icon>下载打开视频软件说明
                </el-button>
                ...
                <el-button
                  ...
                  @click="openLps"
                >
                  <el-icon><Download /></el-icon>启动 Voko 录屏1
                </el-button>
                <a href="voko://open" class="btn">启动 Voko 录屏2</a>
              </div>
            </el-header>
            <div class="tableBox">
              <el-table
...
export default {
  ...
  data() {
    return {
      ...
      url:{
        export:'/wrj/wjbdWrjLpwj/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjLpwj/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    table_download(){
      window.open('/static/打开视频软件说明.zip')
    },
    openLp(){
      if(this.selection.length!=1){
        ElMessage.info('请选择一条数据');
        return
      }
      setTimeout(()=>{
        window.open(`cmd://msPlayerClick.exe$${this.selection[0].qdwjml},1`, '_blank');
      },500)
    },
    openLps(){
      window.open("voko://open", '_blank');
    },
    //配置
    pzEvent(row){
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.lprjcs.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    getData(){
      ...
      window.API.lprjcs.list(params).then(res=>{
        if(res.code == 200){
