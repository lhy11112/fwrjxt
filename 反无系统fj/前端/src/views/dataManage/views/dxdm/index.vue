  ...
  <div style="width:100%;height:100%;">
            <div class="top-input">
              <el-row>
                <el-col :span="6">
                  <label>名称:</label>
                  <el-input v-model="queryInfo.dmmc" size="small" clearable></el-input>
                </el-col>
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('综合兵要参数')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('综合兵要_导入模板','template')"
                >
...
export default {
  ...
  data() {
    return {
      ...
      url:{
        export:'/dxyy/wjbdDxyyDxdm/exportXls',
        import:`${config.API_URL}/dxyy/wjbdDxyyDxdm/importExcel`,
      }
    };
  },
  ...
  methods: {
    handleSuccess(file){
      if(file.success){
        ...
        ElMessage.error(file.message || "导入失败")
      }
    },
    ...
    openLp(){
      ...
      setTimeout(()=>{
        window.open(`cmd://msPlayerClick.exe$${this.selection[0].dz},1`, '_blank');
      },500)
    },
    //配置
    pzEvent(row){
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.dxdm.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    getData(){
      ...
      window.API.dxdm.list(params).then(res=>{
        if(res.code == 200){
