  ...
  <div style="width:100%;height:100%;">
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('编携配装')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('编携配装_导入模板','template')"
                >
...
export default {
  ...
  data() {
    return {
      ...
      url:{
        export:'/wrj/wjbdWrjBxpz/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjBxpz/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.bxpz.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    getData(){
      ...
      window.API.bxpz.list(params).then(res=>{
        if(res.code == 200){
