...
export default {
  ...
  data() {
    return {
      ...
      columnData:[
        { prop: "fl", label: "分类"},
        { prop: "mc", label: "名称" },
        { prop: "gntd", label: "功能特点" },
        { prop: "jszb", label: "技术指标" },
        { prop: "tp", label: "装备图册" },
        { prop: "sl", label: "装备数量" },
        { prop: "zbly", label: "装备来源" },
      ...
      url:{
        export:'/wrj/wjbdWrjFzzb/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjFzzb/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.fzzb.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    getData(){
      ...
      window.API.fzzb.list(params).then(res=>{
        if(res.code == 200){
