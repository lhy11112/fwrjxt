<template>
  ...
  <div class="top-input">
              <el-row>
                <el-col :span="6">
                  <label>事件类型:</label>
                  <el-input v-model="queryInfo.eventType" size="small" clearable></el-input>
                </el-col>
...
export default {
  ...
  data() {
    return {
        czColumnData:[
        // { prop: "stationId", label: "站ID",width:100 },
        { prop: "deviceIp", label: "连接IP" },
        { prop: "eventType", label: "事件类型" },
        { prop: "eventTime", label: "事件时间" },
        { prop: "reason", label: "事件原因" }
      ],
      pageOption:{
        pageNo:1,
        pageSize:10,
  ...
  methods: {
    ...
    getData(){
      ...
      window.API.sbgl.uavConnectLogList(params).then(res=>{
        if(res.code == 200){
          this.tableData = res.result.records;
          // this.tableData.forEach(item=>{
          //   item.eventTime = item.eventTime.year+'-'+item.eventTime.monthValue+'-'+item.eventTime.dayOfMonth+' '+item.eventTime.hour+':'+item.eventTime.minute+':'+item.eventTime.second;
          // })
