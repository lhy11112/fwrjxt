  ...
  <div style="width:100%;height:100%;">
            <div class="top-input">
              <el-row>
                <el-col :span="6">
                  <label>无人机型号:</label>
                  <el-input v-model="queryInfo.wrjxh" size="small" clearable></el-input>
                </el-col>
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('无人机告警')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('无人机告警_导入模板','template')"
                >
...
export default {
  ...
  data() {
    return {
      ...
      url:{
        export:'/wrj/wjbdWrjGjjl/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjGjjl/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.wrjgj.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    inquires(){
      ...
      // if (this.queryInfo.gjlx && !regex.test(this.queryInfo.gjlx)) {
      //   ElMessage.warning('告警类型不能包含特殊字符，且长度不能超过20位')
      //   return
      // }
    ...
    getData(){
      ...
      window.API.wrjgj.list(params).then(res=>{
        if(res.code == 200){
