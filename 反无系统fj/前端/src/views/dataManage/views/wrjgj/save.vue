  ...
  >
    ...
    >
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="告警类型" prop="gjlx">
            <el-input v-model="form.gjlx" :placeholder="'请输入'" clearable>
            </el-input>
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item label="告警发生经度" prop="gjfsjd">
            <el-input v-model="form.gjfsjd" :placeholder="'请输入'" clearable>
            </el-input>
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item label="告警发生纬度" prop="gjfswd">
            <el-input v-model="form.gjfswd" :placeholder="'请输入'" clearable>
            </el-input>
          </el-form-item>
        </el-col>

         <el-col :span="12">
          <el-form-item label="告警发生时间" prop="gjfssj">
            <el-date-picker
              style="width: 100%;"
              v-model="form.gjfssj"
              type="datetime"
        ...
        <el-col :span="12">
          <el-form-item label="处理状态" prop="clzt">
            <el-input v-model="form.clzt" :placeholder="'请输入'" clearable >
            </el-input>
          </el-form-item>
        </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        kyid: [{ required: true, message: "请选择空域" }],
        wrjid: [{ required: true, message: "请选择无人机" }],
        gjlx: [{ required: true, message: "请输入告警类型" }],
      },
      mode: "add",
      titleMap: {
        add: "新增",
    ...
    submit() {
      this.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          if (this.mode == "add") {
            window.API.wrjgj.add(data).then((res) => {
              if (res.code == 200) {
                this.visibleDialog = false;
            ...
            window.API.wrjgj.edit(data).then((res) => {
              if (res.code == 200) {
                this.visibleDialog = false;
                this.$message.success("操作成功")
                this.$emit("successClick");
