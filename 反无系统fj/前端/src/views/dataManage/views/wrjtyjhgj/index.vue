  ...
  <div style="width:100%;height:100%;">
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('无人机推演数据')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('无人机推演数据_导入模板','template')"
                >
...
export default {
  ...
  data() {
    return {
      currentIndex: 0,
      columnData:[
        { prop: "tyjhId", label: "推演计划id", width: 180 },
      ...
      url:{
        export:'/wrj/wjbdWrjTyjhData/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjTyjhData/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.wrjtyjhgj.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    getData(){
      ...
      window.API.wrjtyjhgj.list(params).then(res=>{
        if(res.code == 200){
