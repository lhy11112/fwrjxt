<template>
  <div style="width:100%;height:100%;">
    ...
    <el-header style="padding:0;">
      <div class="left-panel">
        ...
        <el-button
          type="primary"
          @click="table_export('空域')"
        >
          <el-icon><Upload /></el-icon>导出
        </el-button>
        <el-button
          type="primary"
          @click="table_export('空域_导入模板','template')"
        >
        ...
        <!-- <el-table-column fixed="right" label="操作" align="center" width="90">
          <template #default="scope">
            <el-button
              ...
              type="primary"
              size="small"
              @click.prevent="wrjDataGl(scope.row)"
            >
              无人机管理
            </el-button>
          </template>
  ...
  <AnalysisMoveDlg
    :title="(wrjglObj?wrjglObj.mc:'空域') + '无人机管理'"
    style="width: 1100px;height:auto;"
    :visibleDialog="dialog.wrjGlVisible"
    @close="dialog.wrjGlVisible=false"
  >
    <wrjGlDialog ref="wrjGlDialogRef"></wrjGlDialog>
  </AnalysisMoveDlg>
</template>
...
import wrjGlDialog from "./wrjGlDialog/index.vue"
...
export default {
  ...
  data() {
    return {
      ...
      url:{
        export:'/wrj/wjbdWrjKy/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjKy/importExcel`,
      }
    };
  },
  created() {
  ...
  methods: {
    wrjDataGl(row){
      this.wrjglObj = row;
      this.dialog.wrjGlVisible = true
      this.$nextTick(()=>{
        if(row.wjbdWrjKysqVoList) this.$refs.wrjGlDialogRef.getData(row.id)
      })
      console.log(row);
    },
    handleSuccess(file){
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.wrjky.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
