  ...
  <div style="width:100%;height:100%;">
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('无人机飞行数据')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('无人机飞行数据_导入模板','template')"
                >
              ...
              >
                ...
                >
                  <template v-if="item.prop=='dataType'" #default="scope" >
                    {{ scope.row.dataType==0?'无线电测向目标': scope.row.dataType==1?'主动雷达目标':scope.row.dataType==3?'ID协议破解目标':scope.row.dataType==4?'多类型融合目标':''}}
                  </template>
                  <template v-else-if="item.prop=='fusedFlag'" #default="scope" >
                    {{ scope.row.fusedFlag=='0'?'无线电测向目标': scope.row.fusedFlag=='1'?'主动雷达目标':scope.row.fusedFlag=='3'?'ID协议破解目标(仅针对融合数据)':''}}
                  </template>
                </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      columnData:[
        ...
        { prop: "angle", label: "飞机角度" },

        { prop: "dataTime", label: "时间戳（U64转换）" },
        { prop: "createTime", label: "入库时间" },
        { prop: "sd", label: "速度" },
         { prop: "jmlx", label: "解密类型" },
        { prop: "dataType", label: "数据类型" },
        { prop: "fusedFlag", label: "融合标记" },
      ...
      url:{
        export:'/uav/uavDetectMsg/exportXls',
        import:`${config.API_URL}/uav/uavDetectMsg/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.wrjfxsj.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    inquires(){
      ...
      //   ElMessage.warning('机型不能包含特殊字符，且长度不能超过20位')
      //   return
      // }
    ...
    getData(){
      ...
      window.API.wrjfxsj.list(params).then(res=>{
        if(res.code == 200){
