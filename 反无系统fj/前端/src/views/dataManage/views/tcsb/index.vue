  ...
  <div style="width:100%;height:100%;">
            <div class="top-input">
              <el-row>
                <el-col :span="6">
                  <label>人员名称:</label>
                  <el-input v-model="queryInfo.rymc" size="small" clearable></el-input>
                </el-col>
...
export default {
  ...
  data() {
    return {
      currentIndex: 0,
      columnData:[
        { prop: "rylb", label: "人员类别", width: 180 },
        { prop: "rymc", label: "人员名称" },
        { prop: "sylx", label: "使用武器类型" },
        { prop: "zznl", label: "作战能力" },
        { prop: "ryjn", label: "人员作战能力" }
      ...
      url:{
        export:'/wjbdfkzzdfllry/wjbdFkzzDfllRy/exportXls',
        import:`${config.API_URL}/wjbdfkzzdfllry/wjbdFkzzDfllRy/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.dfry.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    getData(){
      ...
      window.API.dfry.list(params).then(res=>{
        if(res.code == 200){
