  ...
  <div style="width:100%;height:100%;">
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('周边目标')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('周边目标_导入模板','template')"
                >
...
export default {
  ...
  data() {
    return {
      ...
      url:{
        export:'/wrj/wjbdWrjZymb/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjZymb/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.zbmb.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    inquires(){
      ...
      //   ElMessage.warning('名称不能包含特殊字符，且长度不能超过20位')
      //   return
      // }
    ...
    getData(){
      ...
      window.API.zbmb.list(params).then(res=>{
        if(res.code == 200){
