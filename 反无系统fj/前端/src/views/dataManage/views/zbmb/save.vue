      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="子类型" prop="subtype">
            <el-select v-model="form.subtype" :placeholder="'请输入'" filterable allow-create clearable>
              <el-option v-for="(item,index) in subtypeOption" :key="index" :label="item.label" :value="item.value"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="人员类别" prop="rylx">
            <el-input v-model="form.rylx" :placeholder="'请输入'" clearable>
            </el-input>
          </el-form-item>
        </el-col>

        <el-col :span="24">
          <el-form-item label="当前位置" prop="wz">
        ...
        <el-col :span="12">
          <el-form-item label="装备" prop="zb">
            <el-input v-model="form.zb" :placeholder="'请输入'" clearable>
            </el-input>
          </el-form-item>
        </el-col>

        <el-col :span="24">
          <el-form-item label="设施概括" prop="ssgk">
            <el-input v-model="form.ssgk" type="textarea" :placeholder="'请输入'" clearable>
            </el-input>
          </el-form-item>
        </el-col>
      </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        wz: [{ required: true,message: "请选择当前位置",trigger:"blur"}],
      },
      deviceTypeOption:[
        {
          label:'侦测',
  ...
  methods: {
    ...
    handleOK(item1) {
      ...
      // this.form.wz = this.ToDegrees(item1.split(",")[0]) + ',' +this.ToDegrees(item1.split(",")[1]);
      ...
      addressMatchService.decode(geoCodeParam, (res)=>{
        if(res.result && res.result.length>0){
          ...
          if(addresses.length && addresses.length==3){
            this.form.wz = addresses[addresses.length-1]+addresses[addresses.length-2]+addresses[addresses.length-3];
          }else{
            this.form.wz = res.result[0].address;
          }
        }else{
          this.form.wz = this.ToDegrees(x) + ',' +this.ToDegrees(y);
        }
      })
    },
    // 经纬度转度分秒
    ...
    setData(data) {
      ...
      if(this.form.wd && this.form.jd){
        ...
        // this.form.wz = this.ToDegrees(data.wd) + ',' + this.ToDegrees(data.jd)
    ...
    submit() {
      this.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          if (this.mode == "add") {
            window.API.zbmb.add(data).then((res) => {
              if (res.code == 200) {
                this.visibleDialog = false;
            ...
            window.API.zbmb.edit(data).then((res) => {
              if (res.code == 200) {
                this.visibleDialog = false;
                this.$message.success("操作成功")
                this.$emit("successClick");
