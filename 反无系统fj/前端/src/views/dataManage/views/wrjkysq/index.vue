  ...
  <div style="width:100%;height:100%;">
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('无人机空域授权')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('无人机空域授权_导入模板','template')"
                >
            ...
            <div class="tableBox">
              ...
              >
                ...
                <el-table-column
                  ...
                  :width="item.width"
                >
                  <template v-if="item.prop == 'sqgqsj'" #default="scope">
                    {{scope.row.sqgqsj==null?'永久':scope.row.sqgqsj}}
                  </template>
                </el-table-column>
...
export default {
  ...
  data() {
    return {
      currentIndex: 0,
      columnData:[
        { prop: "kyid", label: "空域id", width: 180 },
        { prop: "wrjid", label: "无人机id" },
        { prop: "sqsj", label: "授权时间" },
        { prop: "sqgqsj", label: "授权过期时间" }
      ],
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.wrjkysq.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    inquires(){
      ...
      //   ElMessage.warning('空域名称不能包含特殊字符，且长度不能超过20位')
      //   return
      // }
    ...
    getData(){
      ...
      window.API.wrjkysq.list(params).then(res=>{
        if(res.code == 200){
