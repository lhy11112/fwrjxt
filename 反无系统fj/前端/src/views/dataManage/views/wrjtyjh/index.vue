  ...
  <div style="width:100%;height:100%;">
            ...
            <el-header style="padding:0;">
              <div class="left-panel">
                ...
                <el-button
                  type="primary"
                  @click="table_export('无人机推演计划')"
                >
                  <el-icon><Upload /></el-icon>导出
                </el-button>
                <el-button
                  type="primary"
                  @click="table_export('无人机推演计划_导入模板','template')"
                >
...
export default {
  ...
  data() {
    return {
      currentIndex: 0,
      columnData:[
        { prop: "mc", label: "推演计划名称", width: 180 },
        { prop: "rq", label: "推演计划日期" },
        { prop: "jhks", label: "计划开始" },
        { prop: "jhjs", label: "计划结束" },
        { prop: "serial", label: "无人机序列号" },
        { prop: "model", label: "机型" },
        { prop: "brand", label: "品牌" },
        { prop: "tyzt", label: "推演状态" },
      ...
      url:{
        export:'/wrj/wjbdWrjTyjh/exportXls',
        import:`${config.API_URL}/wrj/wjbdWrjTyjh/importExcel`,
      }
    };
  },
  ...
  methods: {
    ...
    batch_del() {
      ...
      ElMessageBox.confirm(`确定删除选中的 ${this.selection.length} 项吗？`, "提示", {
        ...
        .then(() => {
          ...
          window.API.wrjtyjh.delete({ids:ids.join()}).then(res=>{
            if(res.code == 200){
              this.$message.success("操作成功");
    ...
    inquires(){
      ...
      // if (this.queryInfo.mc && !regex.test(this.queryInfo.mc)) {
      //   ElMessage.warning('推演计划名称不能包含特殊字符，且长度不能超过20位')
      //   return
      // }
    ...
    getData(){
      ...
      window.API.wrjtyjh.list(params).then(res=>{
        if(res.code == 200){
